...
def get_station_rad_df(end_dt_str: str, station_code: str) -> pd.DataFrame:
    """
        - 22-04-26
          测试爬取路径:
            ...
            try:
                if len(td_list) >= 2:
                    ...
                    for td_index, td_name in enumerate(td_list):
                        if rad_stamp_str in td_name.text:
...
def main():
    ...
    for station_temp in list_station:
        ...
        for dt_temp in list_year_split_dt:
            ...
            df = None
            df = get_station_rad_df(dt_str, station_temp)
        ...
        if all_data_df is not None:
            ...
            # 存储
            to_store('/opt/project', f'{station_temp}_2021_all.csv', all_data_df)
